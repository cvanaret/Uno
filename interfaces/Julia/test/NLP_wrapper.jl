@testset "preset = $preset" for preset in ("filterslp",)
    @testset "LP solver = $lp" for lp in ("BQPD", "HiGHS")
        @testset "uno_model -- uno_solver -- uno_optimize -- HS15 -- $(hessian_model) Hessian" for hessian_model in ("exact", "LBFGS", "LSR1")
            ...
            model = uno_model(nlp)
            solver = uno_solver(preset=preset, print_solution=true, LP_solver=lp, hessian_model=hessian_model)
        ...
        @testset "uno -- CUTEst -- BYRDSPHR -- $(hessian_model) Hessian" for hessian_model in ("exact", "LBFGS", "LSR1")
            ...
            stats = uno(nlp, preset=preset, print_solution=true, LP_solver=lp, hessian_model=hessian_model)
...
@testset "preset = $preset" for preset in ("filtersqp", "funnelsqp")
    @testset "QP solver = $qp" for qp in ("BQPD",)
        @testset "uno_model -- uno_solver -- uno_optimize -- HS15 -- $(hessian_model) Hessian" for hessian_model in ("exact", "LBFGS", "LSR1")
            nlp = CUTEstModel{Float64}("HS15")
            model = uno_model(nlp)
            solver = uno_solver(preset=preset, print_solution=true, QP_solver=qp, hessian_model=hessian_model)
            uno_optimize(solver, model)
        ...
        @testset "uno -- CUTEst -- BYRDSPHR -- $(hessian_model) Hessian" for hessian_model in ("exact", "LBFGS", "LSR1")
            nlp = CUTEstModel{Float64}("BYRDSPHR")
            stats = uno(nlp, preset=preset, print_solution=true, QP_solver=qp, hessian_model=hessian_model)
            model, solver = stats.model, stats.solver
    ...
    for linear_solver in ("MUMPS", "SSIDS", "MA27", "MA57")
        ...
        @testset "linear solver = $linear_solver" begin
            @testset "uno_model -- uno_solver -- uno_optimize -- HS15 -- $(hessian_model) Hessian" for hessian_model in ("exact", "LBFGS")
                nlp = CUTEstModel{Float64}("HS15")
                model = uno_model(nlp)
                solver = uno_solver(preset=preset, print_solution=true, linear_solver=linear_solver, hessian_model=hessian_model)
                uno_optimize(solver, model)
            ...
            @testset "uno -- CUTEst -- BYRDSPHR -- $(hessian_model) Hessian" for hessian_model in ("exact", "LBFGS")
                nlp = CUTEstModel{Float64}("BYRDSPHR")
                stats = uno(nlp, preset=preset, print_solution=true, linear_solver=linear_solver, hessian_model=hessian_model)
                model, solver = stats.model, stats.solver
