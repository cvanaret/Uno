...
namespace uno {
   void define_Model(py::module& module) {
      ...
      .def("set_lagrangian_hessian", [](PythonUserModel& user_model, uno_int number_hessian_nonzeros, char hessian_triangular_part,
            std::vector<uno_int> hessian_row_indices, std::vector<uno_int> hessian_column_indices,
            Hessian lagrangian_hessian, double lagrangian_sign_convention) {
         ...
         if (hessian_triangular_part == UNO_UPPER_TRIANGLE) {
            // normalize to lower triangle by transposing
            std::swap(hessian_row_indices, hessian_column_indices);
         }
