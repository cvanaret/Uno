...
!---------------------------------------------
abstract interface
    function Objective(number_variables, x, objective_value, user_data) bind(C)
...
!---------------------------------------------
abstract interface
    function Constraints(number_variables, number_constraints, x, constraint_values, user_data) bind(C)
        import :: c_double, c_ptr, uno_int
...
!---------------------------------------------
abstract interface
    function ObjectiveGradient(number_variables, x, gradient, user_data) bind(C)
        import :: c_double, c_ptr, uno_int
...
!---------------------------------------------
abstract interface
    function Jacobian(number_variables, number_jacobian_nonzeros, x, jacobian, &
                      user_data) bind(C)
        import :: c_double, c_ptr, uno_int
...
!---------------------------------------------
abstract interface
    function Hessian(number_variables, number_constraints, number_hessian_nonzeros, x, objective_multiplier, &
                     multipliers, hessian, user_data) bind(C)
        import :: c_double, c_ptr, uno_int
...
!---------------------------------------------
abstract interface
    function JacobianOperator(number_variables, number_constraints, x, evaluate_at_x, &
...
!---------------------------------------------
abstract interface
    function JacobianTransposedOperator(number_variables, number_constraints, x, &
                                        evaluate_at_x, vector, result, &
                                        user_data) bind(C)
        import :: c_double, c_bool, c_ptr, uno_int
...
!---------------------------------------------
abstract interface
    function HessianOperator(number_variables, number_constraints, x, evaluate_at_x, objective_multiplier, &
                             multipliers, vector, result, user_data) bind(C)
        import :: c_double, c_bool, c_ptr, uno_int
...
!---------------------------------------------
abstract interface
    subroutine NotifyAcceptableIterateUserCallback(number_variables, number_constraints, primals, lower_bound_multipliers, &
                                                   upper_bound_multipliers, constraint_multipliers, objective_multiplier, &
...
!---------------------------------------------
abstract interface
    function TerminationUserCallback(number_variables, number_constraints, primals, lower_bound_multipliers, &
                                     upper_bound_multipliers, constraint_multipliers, objective_multiplier, &
                                     primal_feasibility_residual, stationarity_residual, complementarity_residual, &
                                     user_data) bind(C)
        import :: c_double, c_bool, c_ptr, uno_int
...
!---------------------------------------------
abstract interface
    function LoggerStreamUserCallback(buffer, length, user_data) bind(C)
