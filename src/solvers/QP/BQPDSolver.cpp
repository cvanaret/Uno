...
BQPDSolver::BQPDSolver() : QPSolver(), use_fortran(1), kmax_(500), mlp_(1000), mxwk0_(2000000), mxiwk0_(500000), info_(100), alp_(mlp_), lp_(mlp_), k_(0), mode_(COLD_START), iprint_(0), nout_(6), fmin_(-1e20) {
    kktalphac_.alpha = 0;
}
...
SubproblemSolution BQPDSolver::solve_QP(std::vector<Range>& variables_bounds, std::vector<Range>& constraints_bounds, std::map<int, double>& linear_objective, std::vector<std::map<int, double> >& constraints_jacobian, CSCMatrix& hessian, std::vector<double>& x) {
    int n = x.size();
    int m = constraints_jacobian.size();
    this->nhr_ = hessian.number_nonzeros();
    this->nhi_ = hessian.number_nonzeros() + n + 3;
    this->mxws_ = nhr_ + kmax_ * (kmax_ + 9) / 2 + 2 * n + m + mxwk0_;
    this->mxlws_ = nhi_ + kmax_ + mxiwk0_;
    this->ws_.resize(mxws_);
    //lws_.resize(mxlws_);
    std::vector<int> lws_(mxlws_);
    ...
    /* Hessian sparsity */
    lws_[0] = hessian.number_nonzeros() + 1;
    for (int i = 0; i < hessian.number_nonzeros(); i++) {
        lws_[i + 1] = hessian.row_number[i] + this->use_fortran;
    }
    for (unsigned int i = 0; i < hessian.column_start.size(); i++) {
        lws_[hessian.number_nonzeros() + i + 1] = hessian.column_start[i] + this->use_fortran;
    }
    ...
    for (int j = hessian.size; j < n; j++) {
        lws_[i] = last_value + this->use_fortran;
        i++;
    }
    return this->solve_subproblem(variables_bounds, constraints_bounds, linear_objective, constraints_jacobian, x, this->kmax_, lws_);
}
...
SubproblemSolution BQPDSolver::solve_LP(std::vector<Range>& variables_bounds, std::vector<Range>& constraints_bounds, std::map<int, double>& linear_objective, std::vector<std::map<int, double> >& constraints_jacobian, std::vector<double>& x) {
    ...
    return this->solve_subproblem(variables_bounds, constraints_bounds, linear_objective, constraints_jacobian, x, 0, lws_);
}

SubproblemSolution BQPDSolver::solve_subproblem(std::vector<Range>& variables_bounds, std::vector<Range>& constraints_bounds, std::map<int, double>& linear_objective, std::vector<std::map<int, double> >& constraints_jacobian, std::vector<double>& x, int kmax, std::vector<int>& lws_) {
    int n = x.size();
    int m = constraints_jacobian.size();
    ...
    bqpd_(&n, &m, &this->k_, &kmax, jacobian.data(), jacobian_sparsity.data(), x.data(),
            ...
            this->lp_.data(), &this->mlp_, &this->peq_solution_, this->ws_.data(), lws_.data(), &mode,
    ...
    SubproblemSolution solution = this->generate_solution(x, n, m);
    return solution;
}

SubproblemSolution BQPDSolver::generate_solution(std::vector<double>& x, int n, int m) {
    Multipliers multipliers(n, m);
    SubproblemSolution solution(x, multipliers);

    /* active constraints */
    for (int j = 0; j < n - this->k_; j++) {
    ...
    for (int j = n - this->k_; j < n + m; j++) {
        int index = std::abs(this->ls_[j]) - this->use_fortran;

        if (n <= index) { // general constraints
            int constraint_index = index - n;
