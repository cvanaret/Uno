...
SLPEQP::SLPEQP(Problem& problem, std::string LP_solver_name, std::string linear_solver_name, std::string hessian_evaluation_method,
      bool /*use_trust_region*/, bool scale_residuals) : ActiveSetMethod(problem, scale_residuals),
      lp_solver(QPSolverFactory::create(LP_solver_name, problem.number_variables, problem.number_constraints, 0, false)),
      linear_solver(LinearSolverFactory::create(linear_solver_name)), hessian_evaluation(
...
Direction SLPEQP::solve_eqp_(Problem& problem, Iterate& current_iterate, Direction& phase_2_direction, double /*trust_region_radius*/) {
   ...
   int dimension = 2;
   COOMatrix kkt_matrix(dimension, 0, 1);
   kkt_matrix.insert(this->hessian_evaluation->hessian.matrix[2], 0, 0);
   ...
   DEBUG << "LP solution: ";
   print_vector(DEBUG, phase_2_direction.x);
   DEBUG << "Infeasible? " << (phase_2_direction.status == INFEASIBLE) << "\n";
   DEBUG << "EQP solution: ";
   print_vector(DEBUG, solution_EQP);
...
std::vector<Direction> SLPEQP::restore_feasibility(Problem& /*problem*/, Iterate& /*current_iterate*/, Direction& /*phase_2_direction*/,
