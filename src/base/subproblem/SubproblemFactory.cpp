...
SubproblemFactory::create(Problem& problem, const std::string& type, std::map<std::string, std::string>& options, bool use_trust_region,
   ...
   if (type == "SQP") {
      ...
      return std::make_unique<Sl1QP>(problem, options["QP_solver"], options["hessian"], use_trust_region, scale_residuals,
            initial_parameter);
   }
   else if (type == "SLPEQP") {
      return std::make_unique<SLPEQP>(problem, options["QP_solver"], options["linear_solver"], options["hessian"], use_trust_region,
            scale_residuals);
   }
//    else if (type == "SLPEQP") {
//          if (use_trust_region) {
