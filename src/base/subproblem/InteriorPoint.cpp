...
void InteriorPoint::modify_inertia_(COOMatrix& kkt_matrix, int size_first_block, int size_second_block) {
...
SubproblemSolution InteriorPoint::restore_feasibility(Problem& problem, Iterate& current_iterate, SubproblemSolution& phase_II_solution, double trust_region_radius) {
    ...
    /* inertia correction */
    this->modify_inertia_(coo_matrix, current_iterate.x.size(), 0);
