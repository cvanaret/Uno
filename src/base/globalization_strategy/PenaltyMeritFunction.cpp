...
SubproblemSolution PenaltyStrategy::compute_step(Problem& problem, Iterate& current_iterate, double radius) {
	/* stage a: compute the step within trust region */
	SubproblemSolution solution = this->subproblem.compute_l1_penalty_step(problem, current_iterate, radius, this->penalty_parameter, this->penalty_dimensions);
	DEBUG << solution;
	...
	if (0. < this->penalty_parameter) {
		...
		if (linear_model != 0.) {
			...
			SubproblemSolution ideal_solution = this->subproblem.compute_l1_penalty_step(problem, current_iterate, radius, 0., this->penalty_dimensions);
