...
FilterStrategy::FilterStrategy(Subproblem& subproblem, FilterStrategyParameters& strategy_parameters,
      std::map<std::string, std::string>& options) : GlobalizationStrategy(subproblem), filter_optimality(FilterFactory::create(options)),
...
Iterate FilterStrategy::initialize(Statistics& statistics, Problem& problem, std::vector<double>& x, Multipliers& multipliers) {
   ...
   double upper_bound = std::max(this->parameters_.ubd, this->parameters_.fact * first_iterate.feasibility_measure);
   ...
   if (step_norm == 0.) {
      ...
      if (acceptable) {
         ...
         if (acceptable) {
            ...
            /* switching condition */
            if (predicted_reduction < this->parameters_.Delta * std::pow(current_iterate.feasibility_measure, 2)) {
...
void FilterStrategy::switch_phase_(Problem& problem, Direction& direction, Iterate& current_iterate, Iterate& trial_iterate) {
   ...
   if (this->current_phase_ == OPTIMALITY) {
      ...
      /* check whether we can switch from phase I (restoration) to II (optimality) */
            ...
            this->filter_optimality->accept(trial_iterate.feasibility_measure, trial_iterate.optimality_measure)) {
