...
## Main options
- `logger` (string) — "INFO" (levels: SILENT|DISCRETE|WARNING|INFO|DEBUG|DEBUG2|DEBUG3)
- `hessian_model` (string) — "exact" (alternatives: "zero")
- `inertia_correction_strategy` (string) — "primal"
...
- `progress_norm` (string) — "L1" (L1|L2|INF)
- `residual_norm` (string) — "INF" (L1|L2|INF)
...
- `filter_type` (string) — "standard" (standard|nonmonotone)
...
## Solver selection
- `QP_solver` (string) — chosen automatically from available QP solvers if any
- `LP_solver` (string) — chosen automatically from available LP solvers if any
- `linear_solver` (string) — chosen automatically from available linear solvers if any

## BQPD options
- `BQPD_kmax_heuristic` (string) — "filtersqp"
