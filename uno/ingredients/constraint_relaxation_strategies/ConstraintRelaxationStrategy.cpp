...
namespace uno {
   ...
   void ConstraintRelaxationStrategy::compute_feasible_direction(Statistics& statistics, const Model& model, Iterate& current_iterate,
         Direction& direction, const Vector<double>& /*initial_point*/, double trust_region_radius, WarmstartInformation& warmstart_information) {
      //this->inequality_handling_method->set_initial_point(initial_point); // TODO
      std::cout << "CALLING ConstraintRelaxationStrategy::compute_feasible_direction\n";
      this->compute_feasible_direction(statistics, model, current_iterate, direction, trust_region_radius, warmstart_information);
   }
   ...
   std::function<double(double)> ConstraintRelaxationStrategy::compute_predicted_objective_reduction_model(const InequalityHandlingMethod& inequality_handling_method,
