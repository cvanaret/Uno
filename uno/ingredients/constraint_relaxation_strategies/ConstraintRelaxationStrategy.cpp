...
namespace uno {
   ...
   void ConstraintRelaxationStrategy::compute_residuals(const Model& model, Iterate& iterate, Evaluations& evaluations) const {
      // stationarity error (norm of the Lagrangian gradient)
      model.evaluate_lagrangian_gradient(iterate.primals, iterate.multipliers, 1., evaluations, iterate.residuals.lagrangian_gradient);
      ...
      iterate.primal_infeasibility = model.constraint_violation(evaluations.constraints, this->residual_norm);
