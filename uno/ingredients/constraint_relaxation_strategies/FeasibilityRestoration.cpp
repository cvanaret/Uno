...
namespace uno {
   FeasibilityRestoration::FeasibilityRestoration(size_t number_constraints, size_t number_bounds_constraints, const Options& options) :
         ...
         optimality_inequality_handling_method(InequalityHandlingMethodFactory::create(number_bounds_constraints, options)),
         feasibility_inequality_handling_method(InequalityHandlingMethodFactory::create(number_bounds_constraints, options)),
   ...
   void FeasibilityRestoration::initialize(Statistics& statistics, const Model& model, Iterate& initial_iterate, Direction& direction, const Options& options) {
      ...
      this->optimality_inequality_handling_method->initialize(optimality_problem, *this->feasibility_hessian_model);
   ...
   void FeasibilityRestoration::compute_feasible_direction(Statistics& statistics, const Model& model, Iterate& current_iterate,
      ...
      if (this->current_phase == Phase::OPTIMALITY) {
         ...
         try {
            ...
            this->solve_subproblem(statistics, optimality_problem, current_iterate, current_iterate.multipliers, direction,
               *this->optimality_inequality_handling_method, *this->optimality_hessian_model, trust_region_radius, warmstart_information);
      ...
      this->solve_subproblem(statistics, feasibility_problem, current_iterate, current_iterate.feasibility_multipliers, direction,
         *this->feasibility_inequality_handling_method, *this->feasibility_hessian_model, trust_region_radius, warmstart_information);
   ...
   void FeasibilityRestoration::solve_subproblem(Statistics& statistics, const OptimizationProblem& problem, Iterate& current_iterate,
         const Multipliers& current_multipliers, Direction& direction, InequalityHandlingMethod& inequality_handling_method,
         HessianModel& hessian_model, double trust_region_radius, WarmstartInformation& warmstart_information) {
      direction.set_dimensions(problem.number_variables, problem.number_constraints);
      inequality_handling_method.solve(statistics, problem, current_iterate, current_multipliers, direction, hessian_model, trust_region_radius,
   ...
   void FeasibilityRestoration::switch_to_optimality_phase(Iterate& current_iterate, const Model& model, Iterate& trial_iterate, WarmstartInformation& warmstart_information) {
      ...
      this->feasibility_inequality_handling_method->exit_feasibility_problem(optimality_problem, trial_iterate);
   ...
   ProgressMeasures FeasibilityRestoration::compute_predicted_reduction_models(const InequalityHandlingMethod& inequality_handling_method,
      ...
      return {
         ...
         this->compute_predicted_objective_reduction_model(inequality_handling_method, current_iterate, direction.primals, step_length),
         this->optimality_inequality_handling_method->compute_predicted_auxiliary_reduction_model(model, current_iterate, direction.primals, step_length)
      };
   }
   ...
   std::string FeasibilityRestoration::get_strategy_combination() const {
      return "feasibility restoration " + this->globalization_strategy->get_strategy_combination() + " " +
         this->optimality_inequality_handling_method->get_strategy_combination();
   }
