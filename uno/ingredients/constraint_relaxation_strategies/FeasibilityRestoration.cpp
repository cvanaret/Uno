...
namespace uno {
   ...
   void FeasibilityRestoration::initialize(Statistics& statistics, InequalityHandlingMethod& inequality_handling_method, const Model& model,
         Iterate& initial_iterate, Direction& direction, const Options& options) {
      // memory allocation
      this->optimality_subproblem_layer.initialize(model);
      ...
      inequality_handling_method.initialize(feasibility_problem, *this->feasibility_subproblem_layer.hessian_model);
      ...
      inequality_handling_method.generate_initial_iterate(optimality_problem, initial_iterate);
   ...
   void FeasibilityRestoration::compute_feasible_direction(Statistics& statistics, InequalityHandlingMethod& inequality_handling_method,
      ...
      if (this->current_phase == Phase::OPTIMALITY) {
         ...
         try {
            ...
            const OptimizationProblem optimality_problem{model};
            this->solve_subproblem(statistics, inequality_handling_method, optimality_problem, current_iterate, current_iterate.multipliers,
               direction, this->optimality_subproblem_layer, trust_region_radius, warmstart_information);
            if (direction.status == SubproblemStatus::INFEASIBLE) {
               ...
               this->switch_to_feasibility_problem(statistics, inequality_handling_method, globalization_strategy, model, current_iterate,
                  warmstart_information);
               inequality_handling_method.set_initial_point(direction.primals);
            }
            else {
      ...
      feasibility_problem.set_proximal_multiplier(inequality_handling_method.proximal_coefficient(current_iterate));
   ...
   void FeasibilityRestoration::switch_to_feasibility_problem(Statistics& statistics, InequalityHandlingMethod& inequality_handling_method,
         GlobalizationStrategy& globalization_strategy, const Model& model, Iterate& current_iterate, WarmstartInformation& warmstart_information) {
      ...
      inequality_handling_method.set_elastic_variable_values(feasibility_problem, current_iterate);
   ...
   bool FeasibilityRestoration::can_switch_to_optimality_phase(const Iterate& current_iterate, GlobalizationStrategy& globalization_strategy,
   ...
   void FeasibilityRestoration::switch_to_optimality_phase(Iterate& current_iterate, InequalityHandlingMethod& inequality_handling_method,
         GlobalizationStrategy& globalization_strategy, const Model& model, Iterate& trial_iterate, WarmstartInformation& warmstart_information) {
      ...
      inequality_handling_method.exit_feasibility_problem(optimality_problem, trial_iterate);
      // set a cold start in the subproblem solver
      warmstart_information.whole_problem_changed();
   }

   bool FeasibilityRestoration::is_iterate_acceptable(Statistics& statistics, InequalityHandlingMethod& inequality_handling_method,
      ...
      if (this->current_phase == Phase::OPTIMALITY) {
         const OptimizationProblem optimality_problem{model};
         inequality_handling_method.postprocess_iterate(optimality_problem, trial_iterate.primals, trial_iterate.multipliers);
      }
      else {
         const l1RelaxedProblem feasibility_problem{model, 0, this->constraint_violation_coefficient};
         inequality_handling_method.postprocess_iterate(feasibility_problem, trial_iterate.primals, trial_iterate.feasibility_multipliers);
      }
      ...
      if (this->current_phase == Phase::FEASIBILITY_RESTORATION && this->can_switch_to_optimality_phase(current_iterate, globalization_strategy,
         ...
         this->switch_to_optimality_phase(current_iterate, inequality_handling_method, globalization_strategy, model, trial_iterate, warmstart_information);
      }
      else {
         warmstart_information.no_changes();
      }
