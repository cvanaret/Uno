...
void COOEvaluationSpace::initialize_hessian(const Subproblem &subproblem) {
  if (!subproblem.has_hessian_matrix()) {
    throw std::runtime_error(
        "The subproblem does not have an explicit Hessian matrix and cannot be "
        "solved with a direct linear solver");
  }
  ...
  subproblem.compute_regularized_hessian_sparsity(
      this->matrix_row_indices.data(), this->matrix_column_indices.data(),
...
void COOEvaluationSpace::initialize_augmented_system(
  ...
  subproblem.compute_constraint_jacobian_sparsity(
      this->jacobian_row_indices.data(), this->jacobian_column_indices.data(),
      Indexing::C_indexing, MatrixOrder::COLUMN_MAJOR);

  // augmented system
  this->number_hessian_nonzeros = subproblem.number_hessian_nonzeros();
  this->number_matrix_nonzeros =
      subproblem.number_regularized_augmented_system_nonzeros();
  this->matrix_row_indices.resize(this->number_matrix_nonzeros);
  this->matrix_column_indices.resize(this->number_matrix_nonzeros);
  // compute the COO sparse representation
  subproblem.compute_regularized_augmented_matrix_sparsity(
      this->matrix_row_indices.data(), this->matrix_column_indices.data(),
      this->jacobian_row_indices.data(), this->jacobian_column_indices.data(),
      Indexing::Fortran_indexing);
  this->matrix_values.resize(this->number_matrix_nonzeros);
...
void COOEvaluationSpace::evaluate_constraint_jacobian(
    const OptimizationProblem &problem, Iterate &iterate) {
  problem.evaluate_constraint_jacobian(
      iterate, this->matrix_values.data() + this->number_hessian_nonzeros);
}

void COOEvaluationSpace::compute_constraint_jacobian_vector_product(
    const Vector<double> &vector, Vector<double> &result) const {
  result.fill(0.);
  ...
  for (size_t nonzero_index : Range(this->number_jacobian_nonzeros)) {
    const size_t constraint_index =
        static_cast<size_t>(this->jacobian_row_indices[nonzero_index]);
    const size_t variable_index =
        static_cast<size_t>(this->jacobian_column_indices[nonzero_index]);
...
void COOEvaluationSpace::compute_constraint_jacobian_transposed_vector_product(
    const Vector<double> &vector, Vector<double> &result) const {
  result.fill(0.);
  const size_t offset = this->number_hessian_nonzeros;
  for (size_t nonzero_index : Range(this->number_jacobian_nonzeros)) {
...
double COOEvaluationSpace::compute_hessian_quadratic_product(
    const Subproblem & /*subproblem*/,
    const Vector<double> & /*vector*/) const {
  return 0.;
}

void COOEvaluationSpace::set_up_linear_system(
    Statistics &statistics, const Subproblem &subproblem,
    DirectSymmetricIndefiniteLinearSolver<double> &linear_solver,
    const WarmstartInformation &warmstart_information) {
  // evaluate the functions at the current iterate
  if (warmstart_information.objective_changed) {
    subproblem.problem.evaluate_objective_gradient(
        subproblem.current_iterate, this->objective_gradient.data());
  }
  if (warmstart_information.constraints_changed) {
    subproblem.problem.evaluate_constraints(subproblem.current_iterate,
                                            this->constraints);
  }

  if (warmstart_information.objective_changed ||
      warmstart_information.constraints_changed) {
    ...
    subproblem.assemble_augmented_matrix(statistics,
                                         this->matrix_values.data());
    // regularize the augmented matrix (this calls the analysis and the
    // factorization)
    subproblem.regularize_augmented_matrix(
        statistics, this->matrix_values.data(),
        subproblem.dual_regularization_factor(), linear_solver);

    // assemble the RHS
    const COOMatrix jacobian{
        this->jacobian_row_indices.data(), this->jacobian_column_indices.data(),
        this->matrix_values.data() + this->number_hessian_nonzeros};
    subproblem.assemble_augmented_rhs(this->objective_gradient,
                                      this->constraints, jacobian, this->rhs);
  }
}
...
bool COOEvaluationSpace::contains_nan() const {
  if (std::any_of(this->objective_gradient.begin(),
                  this->objective_gradient.end(),
                  [](double v) { return !is_finite(v); }))
    return true;
  if (std::any_of(this->constraints.begin(), this->constraints.end(),
                  [](double v) { return !is_finite(v); }))
    return true;
  if (std::any_of(this->matrix_values.begin(), this->matrix_values.end(),
                  [](double v) { return !is_finite(v); }))
    return true;
  if (std::any_of(this->rhs.begin(), this->rhs.end(),
                  [](double v) { return !is_finite(v); }))
    return true;
  if (std::any_of(this->solution.begin(), this->solution.end(),
                  [](double v) { return !is_finite(v); }))
    return true;
  return false;
}
