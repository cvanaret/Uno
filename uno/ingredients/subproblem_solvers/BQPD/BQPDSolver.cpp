...
namespace uno {
   ...
   void BQPDSolver::initialize_memory(const OptimizationProblem& problem, const HessianModel& hessian_model,
      ...
      // set the maximum size of the nullspace
      this->kmax = (0 < number_regularized_hessian_nonzeros) ? this->kmax_limit : 0;
      if (hessian_model.implicit_representation()) {
         DEBUG << "The Hessian model has an implicit representation. Using Hessian-vector products\n";
      }
      else {
         DEBUG << "The Hessian model has an explicit representation. Using a sparse matrix\n";
         this->hessian = SymmetricMatrix<size_t, double>("CSC", problem.number_variables, number_hessian_nonzeros,
            regularization_size);
      }

      // initialize the workspace without
      this->size_hessian_sparsity = 2 + problem.number_variables + 3;
      this->size_hessian_workspace = 2 + static_cast<size_t>(this->kmax * (this->kmax + 9) / 2) +
   ...
   double BQPDSolver::hessian_quadratic_product(const Vector<double>& vector) const {
      // TODO here we need to know whether the Hessian representation is implicit
   ...
   void BQPDSolver::set_up_subproblem(Statistics& statistics, const Subproblem& subproblem,
      ...
      // if the Hessian has an explicit representation, evaluate it if needed
      if ((warmstart_information.objective_changed || warmstart_information.constraints_changed) &&
            !subproblem.implicit_hessian_representation()) {
         this->hessian.reset();
      ...
      this->hide_pointers_in_workspace(subproblem);
   }
   ...
   void BQPDSolver::hide_pointers_in_workspace(const Subproblem& subproblem) {
      WSC.kk = 0; // length of ws that is used by gdotx
      WSC.ll = 0; // length of lws that is used by gdotx

      // hide pointers to subproblem and explicit Hessian in lws
      intptr_t pointer_to_subproblem = reinterpret_cast<intptr_t>(&subproblem);
      std::copy(reinterpret_cast<const char *>(&pointer_to_subproblem),
         reinterpret_cast<const char *>(&pointer_to_subproblem) + sizeof(intptr_t),
         reinterpret_cast<char *>(this->workspace_sparsity.data()));
      WSC.ll += sizeof(intptr_t);

      intptr_t pointer_to_explicit_hessian = reinterpret_cast<intptr_t>(&this->hessian);
      std::copy(reinterpret_cast<const char *>(&pointer_to_explicit_hessian),
         reinterpret_cast<const char *>(&pointer_to_explicit_hessian) + sizeof(intptr_t),
...
void hessian_vector_product(int* dimension, const double vector[], const double /*ws*/[], const int lws[], double result[]) {
   ...
   // retrieve subproblem and explicit Hessian
   intptr_t pointer_to_subproblem;
   std::copy(reinterpret_cast<const char *>(lws),
      reinterpret_cast<const char *>(lws) + sizeof(intptr_t),
      reinterpret_cast<char *>(&pointer_to_subproblem));
   const uno::Subproblem* subproblem = reinterpret_cast<const uno::Subproblem*>(pointer_to_subproblem);
   assert(subproblem != nullptr && "BQPD's hessian_vector_product: the subproblem is NULL");

   // convert x[] and v[] into Vector<double>
   // TODO improve that
   uno::Vector<double> uno_vector(subproblem->number_variables);
   for (size_t variable_index: uno::Range(subproblem->number_variables)) {
      uno_vector[variable_index] = vector[variable_index];
   }
   uno::Vector<double> uno_result(subproblem->number_variables);

   if (subproblem->implicit_hessian_representation()) {
      // if implicit Hessian representation, compute a Hessian-vector product directly
      subproblem->compute_hessian_vector_product(uno_vector, uno_result);
   }
   else {
      // if explicit Hessian representation, retrieve the pointer to the Hessian
      intptr_t pointer_to_explicit_hessian;
      std::copy(reinterpret_cast<const char *>(lws) + sizeof(intptr_t),
         reinterpret_cast<const char *>(lws) + 2*sizeof(intptr_t),
         reinterpret_cast<char *>(&pointer_to_explicit_hessian));
      uno::SymmetricMatrix<size_t, double>* hessian = reinterpret_cast<uno::SymmetricMatrix<size_t, double>*>(pointer_to_explicit_hessian);
      assert(hessian != nullptr && "BQPD's hessian_vector_product: the Hessian is NULL");
      hessian->product(uno_vector, uno_result);
   }

   for (size_t variable_index: uno::Range(subproblem->number_variables)) {
      result[variable_index] = uno_result[variable_index];
   }
}