...
void FilterStrategy::register_current_progress(const ProgressMeasures& current_progress) {
   this->filter->add(current_progress.infeasibility, current_progress.scaled_optimality);
}
...
bool FilterStrategy::is_acceptable(const ProgressMeasures& current_progress, const ProgressMeasures& trial_progress, double /*objective_multiplier*/,
   ...
   bool accept = false;
   // check acceptance
   const bool filter_acceptable = this->filter->accept(trial_progress.infeasibility, trial_progress.scaled_optimality);
   if (filter_acceptable) {
      DEBUG << "Filter acceptable\n";
      DEBUG << *this->filter << '\n';

      // check acceptance wrt current x (h,f)
      const bool improves_current_iterate = this->filter->improves_current_iterate(current_progress.infeasibility, current_progress.scaled_optimality,
            trial_progress.infeasibility, trial_progress.scaled_optimality);
      if (improves_current_iterate) {
         ...
         const double actual_reduction = this->filter->compute_actual_reduction(current_progress.scaled_optimality, current_progress.infeasibility,
               trial_progress.scaled_optimality);
