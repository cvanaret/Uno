// Copyright (c) 2024 David Kiessling
...
namespace uno {
   ...
   bool FunnelMethod::is_infeasibility_acceptable_to_funnel(double infeasibility_measure) const {
      return (infeasibility_measure <= this->funnel_width);
   }

   /* check if the funnel sufficient decrease condition is satisfied */
   bool FunnelMethod::funnel_sufficient_decrease_condition(double infeasibility_measure) const {
      return (infeasibility_measure <= this->parameters.beta * this->funnel_width);
   }

   bool FunnelMethod::is_infeasibility_sufficiently_reduced(const ProgressMeasures& reference_progress,
   ...
   void FunnelMethod::update_funnel_width(double current_infeasibility_measure, double trial_infeasibility_measure) {
      if (this->parameters.update_strategy == 1) {
         if (trial_infeasibility_measure <= current_infeasibility_measure) {
            this->funnel_width = std::max(this->parameters.beta * this->funnel_width,
                  FunnelMethod::convex_combination(current_infeasibility_measure, trial_infeasibility_measure, this->parameters.kappa));
         }
         else {
            DEBUG << "Trial infeasibility higher than current infeasibility" << "\n";
            this->funnel_width = this->parameters.beta * this->funnel_width;
         }
      }
      else if (this->parameters.update_strategy == 2) {
         this->funnel_width = FunnelMethod::convex_combination(this->funnel_width, trial_infeasibility_measure, this->parameters.kappa);
      }
      else if (this->parameters.update_strategy == 3) {
         this->funnel_width = this->parameters.beta * this->funnel_width;
      }
   ...
   void FunnelMethod::update_funnel_width_restoration(double current_infeasibility_measure) {
      this->funnel_width = FunnelMethod::convex_combination(this->funnel_width, current_infeasibility_measure, this->parameters.kappa);
      DEBUG << "\t\tNew funnel parameter is: " << this->funnel_width << "\n";
   }
   ...
   bool FunnelMethod::is_regular_iterate_acceptable(Statistics& statistics, const ProgressMeasures& current_progress,
      ...
      if (this->is_infeasibility_acceptable_to_funnel(trial_progress.infeasibility)) {
         ...
         if (not this->check_for_current_iterate ||
            ...
            if (this->switching_condition(merit_predicted_reduction, current_progress.infeasibility, this->parameters.delta)) {
               ...
               // h-type step
            else if (this->funnel_sufficient_decrease_condition(trial_progress.infeasibility)) {
