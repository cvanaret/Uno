...
namespace uno {
      ...
      if (!this->model.get_slacks().is_empty()) {
         ...
         for (const auto [constraint_index, slack_index]: this->model.get_slacks()) {
            initial_iterate.primals[slack_index] =
               ...
               this->reformulated_problem.variable_lower_bound(slack_index), this->reformulated_problem.variable_upper_bound(slack_index));
         }
   ...
   void PrimalDualInteriorPointProblem::evaluate_objective_gradient(Iterate& iterate, const EvaluationSpace& evaluation_space,
      ...
      for (size_t variable_index: Range(this->reformulated_problem.number_variables)) {
         ...
         if (is_finite(this->reformulated_problem.variable_lower_bound(variable_index))) { // lower bounded
            barrier_term += -this->barrier_parameter/(iterate.primals[variable_index] - this->reformulated_problem.variable_lower_bound(variable_index));
            // damping
         ...
         if (is_finite(this->reformulated_problem.variable_upper_bound(variable_index))) { // upper bounded
            barrier_term += -this->barrier_parameter/(iterate.primals[variable_index] - this->reformulated_problem.variable_upper_bound(variable_index));
            // damping
   ...
   void PrimalDualInteriorPointProblem::compute_hessian_sparsity(const HessianModel& hessian_model, int* row_indices,
      ...
      size_t current_index = this->reformulated_problem.number_hessian_nonzeros(hessian_model);
      for (size_t variable_index: Range(this->reformulated_problem.number_variables)) {
   ...
   void PrimalDualInteriorPointProblem::evaluate_lagrangian_hessian(Statistics& statistics, HessianModel& hessian_model, const Vector<double>& primal_variables,
      ...
      // barrier terms
      size_t nonzero_index = this->reformulated_problem.number_hessian_nonzeros(hessian_model);
      for (size_t variable_index: Range(this->reformulated_problem.number_variables)) {
         const bool finite_lower_bound = is_finite(this->reformulated_problem.variable_lower_bound(variable_index));
         const bool finite_upper_bound = is_finite(this->reformulated_problem.variable_upper_bound(variable_index));
         if (finite_lower_bound || finite_upper_bound) {
   ...
   void PrimalDualInteriorPointProblem::compute_hessian_vector_product(HessianModel& hessian_model, const double* x,
      ...
      // original Lagrangian Hessian
      this->reformulated_problem.compute_hessian_vector_product(hessian_model, x, vector, multipliers, result);
   ...
   [[nodiscard]] const Collection<size_t>& PrimalDualInteriorPointProblem::get_primal_regularization_variables() const {
      ...
      const VectorExpression shifted_bound_complementarity{variables_range, [&](size_t variable_index) {
         ...
         if (0. < multipliers.lower_bounds[variable_index]) { // lower bound
            result = std::max(result, std::abs(multipliers.lower_bounds[variable_index] *
               (primals[variable_index] - this->reformulated_problem.variable_lower_bound(variable_index)) - shift));
         }
         if (multipliers.upper_bounds[variable_index] < 0.) { // upper bound
            result = std::max(result, std::abs(multipliers.upper_bounds[variable_index] *
               (primals[variable_index] - this->reformulated_problem.variable_upper_bound(variable_index)) - shift));
         }
         return result;
      }};
