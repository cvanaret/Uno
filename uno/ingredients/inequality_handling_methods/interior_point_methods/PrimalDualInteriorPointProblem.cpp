...
namespace uno {
   ...
   void PrimalDualInteriorPointProblem::generate_initial_iterate(Iterate& initial_iterate) const {
      ...
      for (size_t variable_index: Range(problem.number_variables)) {
         initial_iterate.primals[variable_index] = this->push_variable_to_interior(initial_iterate.primals[variable_index],
            this->problem_variable_lower_bound(variable_index), this->problem_variable_upper_bound(variable_index));
      }
      ...
      if (!problem.model.get_slacks().is_empty()) {
         ...
         for (const auto [constraint_index, slack_index]: problem.model.get_slacks()) {
            initial_iterate.primals[slack_index] =
               ...
               this->problem_variable_lower_bound(slack_index), this->problem_variable_upper_bound(slack_index));
         }
      ...
      for (size_t variable_index: Range(problem.number_variables)) {
         const double lower_bound = this->problem_variable_lower_bound(variable_index);
         const double upper_bound = this->problem_variable_upper_bound(variable_index);
   ...
   void PrimalDualInteriorPointProblem::evaluate_objective_gradient(Iterate& iterate, double* objective_gradient) const {
      ...
      for (size_t variable_index: Range(this->problem.number_variables)) {
         ...
         if (is_finite(this->problem_variable_lower_bound(variable_index))) { // lower bounded
            barrier_term += -this->barrier_parameter/(iterate.primals[variable_index] - this->problem_variable_lower_bound(variable_index));
            // damping
            if (!is_finite(this->problem_variable_upper_bound(variable_index))) {
               barrier_term += this->parameters.damping_factor * this->barrier_parameter;
            }
         }
         if (is_finite(this->problem_variable_upper_bound(variable_index))) { // upper bounded
            barrier_term += -this->barrier_parameter/(iterate.primals[variable_index] - this->problem_variable_upper_bound(variable_index));
            // damping
            if (!is_finite(this->problem_variable_lower_bound(variable_index))) {
               barrier_term -= this->parameters.damping_factor * this->barrier_parameter;
            }
         }
   ...
   void PrimalDualInteriorPointProblem::compute_hessian_sparsity(const HessianModel& hessian_model, int* row_indices,
      ...
      size_t current_index = this->problem.number_hessian_nonzeros(hessian_model);
      for (size_t variable_index: Range(this->problem.number_variables)) {
         const bool finite_lower_bound = is_finite(this->problem_variable_lower_bound(variable_index));
         const bool finite_upper_bound = is_finite(this->problem_variable_upper_bound(variable_index));
   ...
   bool PrimalDualInteriorPointProblem::has_curvature(const HessianModel& hessian_model) const {
      if (hessian_model.has_curvature(this->model)) {
         ...
         // barrier terms
         for (size_t variable_index: Range(this->problem.number_variables)) {
            if (is_finite(this->problem_variable_lower_bound(variable_index)) || is_finite(this->problem_variable_upper_bound(variable_index))) {
               return true;
            }
         }
         return false;
   ...
   size_t PrimalDualInteriorPointProblem::number_hessian_nonzeros(const HessianModel& hessian_model) const {
      size_t number_nonzeros = this->problem.number_hessian_nonzeros(hessian_model);
   ...
   void PrimalDualInteriorPointProblem::evaluate_lagrangian_gradient(LagrangianGradient<double>& lagrangian_gradient,
      ...
      this->problem.evaluate_lagrangian_gradient(lagrangian_gradient, inequality_handling_method, iterate);
   ...
   void PrimalDualInteriorPointProblem::evaluate_lagrangian_hessian(Statistics& statistics, HessianModel& hessian_model, const Vector<double>& primal_variables,
      ...
      // barrier terms
      size_t nonzero_index = this->problem.number_hessian_nonzeros(hessian_model);
      for (size_t variable_index: Range(this->problem.number_variables)) {
         const bool finite_lower_bound = is_finite(this->problem_variable_lower_bound(variable_index));
         const bool finite_upper_bound = is_finite(this->problem_variable_upper_bound(variable_index));
         if (finite_lower_bound || finite_upper_bound) {
            ...
            if (finite_lower_bound) {
               const double distance_to_bound = primal_variables[variable_index] - this->problem_variable_lower_bound(variable_index);
               diagonal_barrier_term += multipliers.lower_bounds[variable_index] / distance_to_bound;
            }
            if (finite_upper_bound) {
               const double distance_to_bound = primal_variables[variable_index] - this->problem_variable_upper_bound(variable_index);
      ...
      for (size_t variable_index: Range(this->problem.number_variables)) {
         ...
         if (finite_lower_bound || finite_upper_bound) {
            ...
            if (finite_lower_bound) { // lower bounded
               const double distance_to_bound = vector[variable_index] - this->problem_variable_lower_bound(variable_index);
               diagonal_barrier_term += multipliers.lower_bounds[variable_index] / distance_to_bound;
            }
            if (finite_upper_bound) { // upper bounded
               const double distance_to_bound = vector[variable_index] - this->problem_variable_upper_bound(variable_index);
               diagonal_barrier_term += multipliers.upper_bounds[variable_index] / distance_to_bound;
            }
   ...
   double PrimalDualInteriorPointProblem::problem_variable_lower_bound(size_t variable_index) const {
      const double lower_bound = this->problem.variable_lower_bound(variable_index);
      return lower_bound - this->parameters.bound_relaxation_factor * std::max(1., std::abs(lower_bound));
   }

   double PrimalDualInteriorPointProblem::problem_variable_upper_bound(size_t variable_index) const {
      const double upper_bound = this->problem.variable_upper_bound(variable_index);
      return upper_bound + this->parameters.bound_relaxation_factor * std::max(1., std::abs(upper_bound));
   }
      ...
      const VectorExpression shifted_bound_complementarity{variables_range, [&](size_t variable_index) {
         ...
         if (0. < multipliers.lower_bounds[variable_index]) { // lower bound
            result = std::max(result, std::abs(multipliers.lower_bounds[variable_index] *
               (primals[variable_index] - this->problem_variable_lower_bound(variable_index)) - shift));
         }
         if (multipliers.upper_bounds[variable_index] < 0.) { // upper bound
            result = std::max(result, std::abs(multipliers.upper_bounds[variable_index] *
               (primals[variable_index] - this->problem_variable_upper_bound(variable_index)) - shift));
         }
         return result;
      }};
