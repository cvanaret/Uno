  ...
  for (const auto &[option_name, option_type] : defaults.option_types) {
    ...
    try {
      ...
      case OptionType::INTEGER:
        std::cout << "INTEGER\t";
        if (defaults.integer_options.find(option_name) !=
            defaults.integer_options.end())
      ...
      case OptionType::DOUBLE:
        std::cout << "DOUBLE\t";
        if (defaults.double_options.find(option_name) !=
            defaults.double_options.end())
      ...
      case OptionType::BOOL:
        std::cout << "BOOL\t";
        if (defaults.bool_options.find(option_name) !=
            defaults.bool_options.end())
      ...
      case OptionType::STRING:
        std::cout << "STRING\t";
        if (defaults.string_options.find(option_name) !=
            defaults.string_options.end())
...
void Options::set_string(const std::string &option_name,
...
void Options::set(const std::string &option_name,
                  const std::string &option_value, bool flag_as_overwritten) {
  try {
    ...
    throw std::out_of_range("The type of the option with name " + option_name +
                            " could not be found");
  }
}
...
uno_int Options::get_int(const std::string &option_name) const {
  ...
  try {
    ...
    throw std::out_of_range("The option with name " + option_name +
                            " was not found");
  }
}
...
Options::get_string_optional(const std::string &option_name) const {
  try {
...
Options Options::get_command_line_options(int argc, char *argv[],
  ...
  for (size_t i = offset; i < static_cast<size_t>(argc); ++i) {
    ...
    if (position == std::string::npos) {
      throw std::runtime_error("The option " + argument +
                               " does not contain the delimiter " + delimiter);
    }
...
Options Options::load_option_file(const std::string &file_name) {
  ...
  file.open(file_name);
  if (!file) {
    throw std::invalid_argument("The option file " + file_name +
                                " was not found");
  } else {
...
void Options::overwrite_with(const Options &overwriting_options) {
  for (const auto &[option_name, option_value] :
       overwriting_options.integer_options) {
    this->integer_options[option_name] = option_value;
  }
  for (const auto &[option_name, option_value] :
       overwriting_options.double_options) {
    this->double_options[option_name] = option_value;
  }
  for (const auto &[option_name, option_value] :
       overwriting_options.bool_options) {
    this->bool_options[option_name] = option_value;
  }
  for (const auto &[option_name, option_value] :
       overwriting_options.string_options) {
    this->string_options[option_name] = option_value;
  }
  ...
  // bool flag_as_overwritten = (existing_option_value.has_value() &&
  // *existing_option_value != option_value); this->set(option_name,
  // option_value, flag_as_overwritten);
}
