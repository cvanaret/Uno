...
double Model::constraint_violation(const std::vector<double>& constraints, Norm residual_norm) const {
   const Range range = Range(constraints.size());
   VectorExpression<double> constraint_violation(range, [&](size_t constraint_index) {
...
double Model::linearized_constraint_violation(const std::vector<double>& primal_direction, const std::vector<double>& constraints,
   ...
   const Range range = Range(this->number_constraints);
   VectorExpression<double> linearized_constraints(range, [&](size_t constraint_index) {
