...
namespace uno {
   ...
   void Iterate::evaluate_constraints(const Model& model) {
      if (!this->are_constraints_computed) {
         if (model.is_constrained()) {
            ...
            // check finiteness
            if (std::any_of(this->model_evaluations.constraints.begin(), this->model_evaluations.constraints.end(), [](double constraint_j) {
   ...
   void Iterate::evaluate_objective_gradient(const Model& model) {
      if (!this->is_objective_gradient_computed) {
         this->model_evaluations.objective_gradient.fill(0.);
