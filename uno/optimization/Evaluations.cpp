...
namespace uno {
   ...
   void Evaluations::compute_jacobian_vector_product([[maybe_unused]] const Model& model, View<const double> vector,
   ...
   void Evaluations::compute_jacobian_transposed_vector_product([[maybe_unused]] const Model& model, View<const double> vector,
         View<double> result) const {
      const size_t number_jacobian_nonzeros = this->jacobian_sparsity->row_indices.size();
      for (size_t nonzero_index: Range(number_jacobian_nonzeros)) {
         const size_t constraint_index = static_cast<size_t>(this->jacobian_sparsity->row_indices[nonzero_index]);
