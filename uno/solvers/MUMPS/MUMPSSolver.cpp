...
namespace uno {
   MUMPSSolver::MUMPSSolver(size_t dimension, size_t number_nonzeros) : DirectSymmetricIndefiniteLinearSolver<size_t, double>(dimension),
         COO_matrix(dimension, number_nonzeros, false, "COO") {
   ...
   void MUMPSSolver::do_symbolic_factorization(const SymmetricMatrix<size_t, double>& matrix) {
      ...
      // get storage
      const auto coo_storage = (COOSparseStorage<int, double>*) (this->COO_matrix.get_sparse_storage());
      this->mumps_structure.irn = coo_storage->row_indices_pointer();
      this->mumps_structure.jcn = coo_storage->column_indices_pointer();
      dmumps_c(&this->mumps_structure);
   }
   ...
   void MUMPSSolver::do_numerical_factorization(const SymmetricMatrix<size_t, double>& /*matrix*/) {
      ...
      const auto coo_storage = (COOSparseStorage<int, double>*) (this->COO_matrix.get_sparse_storage());
      this->mumps_structure.a = coo_storage->data_pointer();
      dmumps_c(&this->mumps_structure);
   }
   ...
   void MUMPSSolver::solve_indefinite_system(const PrimalDualInteriorPointSystem& /*linear_system*/,
      ...
      throw std::runtime_error("MUMPSSolver::solve_indefinite_system not implemented for PrimalDualInteriorPointSystem");
   }
