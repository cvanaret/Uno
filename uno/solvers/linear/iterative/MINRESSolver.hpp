...
namespace uno {
   ...
   public:
      explicit MINRESSolver(size_t dimension, NumericalType tolerance);
   ...
   private:
      ...
      // functions
      [[nodiscard]] std::pair<NumericalType, NumericalType> compute_lanczos_step(const LinearOperator& linear_operator, NumericalType beta_k);
      [[nodiscard]] NumericalType compute_symmetric_orthogonalization(NumericalType a, NumericalType b);
   };

   template <typename IndexType, typename NumericalType, typename LinearOperator>
   MINRESSolver<IndexType, NumericalType, LinearOperator>::MINRESSolver(size_t dimension, NumericalType tolerance):
         IterativeSymmetricIndefiniteLinearSolver<IndexType, NumericalType, LinearOperator>(dimension),
         tolerance(tolerance),
         p_k(dimension),
         v_km1(dimension), v_k(dimension),
         d_km2(dimension), d_km1(dimension), d_k(dimension),
         x_k(dimension) {
   }

   // TODO: user-defined termination criterion
   template <typename IndexType, typename NumericalType, typename LinearOperator>
   void MINRESSolver<IndexType, NumericalType, LinearOperator>::solve_indefinite_system(const LinearOperator& linear_operator,
         const Vector<NumericalType>& rhs, Vector<NumericalType>& result) {
      this->iteration = 1;
      // v_0 = 0
