...
namespace uno {
   ...
   void Model::partition_variables(Vector<size_t>& fixed_variables, std::vector<size_t>& lower_bounded_variables,
      ...
      for (size_t variable_index: Range(this->number_variables)) {
         ...
         if (lower_bound == upper_bound) {
            ...
            lower_bounded_variables.emplace_back(variable_index);
            upper_bounded_variables.emplace_back(variable_index);
         }
         else if (is_finite(lower_bound)) {
            lower_bounded_variables.emplace_back(variable_index);
            single_lower_bounded_variables.emplace_back(variable_index);
         }
         else if (is_finite(upper_bound)) {
            upper_bounded_variables.emplace_back(variable_index);
            single_upper_bounded_variables.emplace_back(variable_index);
         }
      }
   }
