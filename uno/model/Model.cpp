   ...
   double Model::complementarity_error(const Vector<double>& primals, const Vector<double>& constraints,
      ...
      const VectorExpression variable_complementarity{variables_range, [&](size_t variable_index) {
         if (0. < multipliers.lower_bounds[variable_index]) {
            /*
            std::cout << "complementarity_error: x" << variable_index << " has lower bound: ";
            std::cout << multipliers.lower_bounds[variable_index] << "*(" << primals[variable_index] << " - " <<
               variables_lower_bounds[variable_index] << ") = " <<
               multipliers.lower_bounds[variable_index] * (primals[variable_index] - variables_lower_bounds[variable_index]) << " \n";
               */
         ...
         if (multipliers.upper_bounds[variable_index] < 0.) {
            /*
            std::cout << "complementarity_error: x" << variable_index << " has upper bound: ";
            std::cout << multipliers.upper_bounds[variable_index] << "*(" << primals[variable_index] << " - " <<
               variables_upper_bounds[variable_index] << ") = " <<
               multipliers.upper_bounds[variable_index] * (primals[variable_index] - variables_upper_bounds[variable_index]) << "\n";
               */
      ...
      const VectorExpression constraint_complementarity{this->get_inequality_constraints(), [&](size_t constraint_index) {
         ...
         if (is_finite(constraints_lower_bounds[constraint_index]) && is_infinite(constraints_upper_bounds[constraint_index])) {
            /*
            std::cout << "complementarity_error: c" << constraint_index << " has lower bound: ";
            std::cout << multipliers.constraints[constraint_index] << "*(" << constraints[constraint_index] << " - " <<
               constraints_lower_bounds[constraint_index] << ") = " <<
               multipliers.constraints[constraint_index] * (constraints[constraint_index] - constraints_lower_bounds[constraint_index]) << " \n";
               */
         ...
         if (is_finite(constraints_upper_bounds[constraint_index]) && is_infinite(constraints_lower_bounds[constraint_index])) {
            /*
            std::cout << "complementarity_error: c" << constraint_index << " has upper bound: ";
            std::cout << multipliers.constraints[constraint_index] << "*(" << constraints[constraint_index] << " - " <<
               constraints_upper_bounds[constraint_index] << ") = " <<
               multipliers.constraints[constraint_index] * (constraints[constraint_index] - constraints_upper_bounds[constraint_index]) << " \n";
               */
