...
namespace uno {
   ...
   void Scaling::compute(const Vector<double>& objective_gradient, const COOMatrix<size_t>& constraint_jacobian) {
      // objective
      ...
      for (size_t constraint_index: Range(this->constraint_scaling.size())) {
         // TODO this->constraint_scaling[constraint_index] = std::min(1., this->gradient_threshold / norm_inf(constraint_jacobian[constraint_index]));
      }
